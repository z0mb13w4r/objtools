...
typedef struct prpsinfo_mips_s {
  uint8_t        pr_state;
  char           pr_sname;
  uint8_t        pr_zomb;
  signed char    pr_nice;
  ...
  char           pr_fname[16];
  char           pr_psargs[80];
} prpsinfo_mips_t, *pprpsinfo_mips_t;
...
const char* ecget_secnamebyaddr(const pbuffer_t p, const uint64_t vaddr);
...
ppick_t get_PUBLICTAGDEC(const pbuffer_t p);
ppick_t get_PUBLICTAGBYTES(const pbuffer_t p);
ppick_t get_PUBLICTAGSTRING(const pbuffer_t p);
...
pconvert_t get_PUBLICTAG(const pbuffer_t p);
