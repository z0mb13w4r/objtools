...
uchar_t crc8_update(uchar_t crc8, uchar_t next) {
  if (next & 0x01) crc8 ^= 0x01;
  if (next & 0x02) crc8 ^= 0x01;
  if (next & 0x04) crc8 ^= 0x01;
  if (next & 0x08) crc8 ^= 0x01;
  if (next & 0x10) crc8 ^= 0x01;
  if (next & 0x20) crc8 ^= 0x01;
  if (next & 0x40) crc8 ^= 0x01;
  if (next & 0x80) crc8 ^= 0x01;
...
ushort_t crc16_update(ushort_t crc16, uchar_t next) {
  return CAST(ushort_t, crc16_table[(crc16 & 0xff) ^ next] ^ (crc16 >> 8));
}

ushort_t crc16_calculate(ushort_t seed16, puchar_t data, size_t datasize) {
  ushort_t crc16 = seed16;
  for (size_t i = 0; i < datasize; ++i) {
...
ulong_t crc32_update(ulong_t crc32, ulong_t next) {
  return CAST(ulong_t, crc32_table[((crc32 ^ next) & 0xff)] ^ (crc32 >> 8));
}

ulong_t crc32_calculate(ulong_t seed32, puchar_t data, size_t datasize) {
  ulong_t crc32 = seed32;

  for (size_t i = 0; i < datasize; ++i) {
    crc32 = crc32_update(crc32, data[i]);
  }

  return crc32 ^ seed32;
}

bool_t csc_check_crc(ushort_t seed16, puchar_t data, size_t size) {
  if (size > 2) {
    const ushort_t crc = csc_compute_crc(seed16, data, size - 2);

    const uchar_t t1 = data[size - 2];
    const uchar_t t2 = data[size - 1];

    if ((t1 == (crc & 0xff)) && (t2 == ((crc >> 8) & 0xff))) return TRUE;
  }
...
ushort_t csc_update_crc(uchar_t next, ushort_t crc) {
  next = next ^ CAST(uchar_t, crc & 0x00FF);
  next = next ^ (next << 4);

  return (crc >> 8) ^ CAST(ushort_t, next << 8) ^ CAST(ushort_t, next << 3) ^ CAST(ushort_t, next >> 4);
}
...
ushort_t csc_compute_crc(ushort_t seed16, puchar_t data, size_t datasize) {
  ...
  if (seed16 != 0) {
    for (size_t i = 0; i < datasize; ++i) {
      crc = csc_update_crc(data[i], crc);
    }
  }

  if (seed16 == CRC_ISO_3309) {
    crc = ~crc; // CRC_B
  }
...
void csc_compute_append_crc(ushort_t seed16, puchar_t data, size_t datasize) {
  const ushort_t crc = csc_compute_crc(seed16, data, datasize);
  data[datasize + 0] = CAST(uchar_t, (crc << 0) & 0xff);
  data[datasize + 1] = CAST(uchar_t, (crc >> 8) & 0xff);
}
