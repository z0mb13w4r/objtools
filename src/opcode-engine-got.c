  ...
  if (pp) {
    ...
    for (uint64_t i = 0; i < ocget_size(s); ) {
      ...
      if (0xf3 == pp[i + 0] && 0x0f == pp[i + 1] && 0x1e == pp[i + 2] && 0xfa == pp[i + 3]) {
        ...
        printf(" %04lx", curr_vaddr);
        printf(" nopl %02x %02x %02x %02x %02x #%04lx\n", pp[i + 0], pp[i + 1], pp[i + 2], pp[i + 3], pp[i + 4], curr_vaddr + prev_vaddr1);
        siz = 5;
        ...
        printf(" %04lx", curr_vaddr);
        printf(" bnd jmpq %02x %02x %02x %08x\n", pp[i + 0], pp[i + 1], pp[i + 2], prev_vaddr1);
        siz = 7;
      } else if (0xf2 == pp[i + 0] && 0xe9 == pp[i + 1]) {
        uint32_t this_vaddr = execute_u32(p, pp[i + 2], pp[i + 3], pp[i + 4], pp[i + 5]);
        printf(" %04lx", curr_vaddr);
        printf(" bnd jmpq %02x %02x %08x\n", pp[i + 0], pp[i + 1], this_vaddr);
        siz = 6;
        ...
        uint32_t this_vaddr = execute_u32(p, pp[i + 1], pp[i + 2], pp[i + 3], pp[i + 4]);
        printf(" %04lx", curr_vaddr);
