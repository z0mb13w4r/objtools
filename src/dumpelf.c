...
int dumpelf_archeader(const pbuffer_t p, const poptions_t o, const char* name) {
  ...
  if (name) {
    n += printf_text(name, USE_LT | USE_RB);
  }
  n += printf_eol();
...
static int dump_fileheader0(const pbuffer_t p, const poptions_t o,
                     const uint64_t e_type, const uint64_t e_machine, const uint64_t e_version,
...
static int dump_programheaders3(const pbuffer_t p, const uint64_t p_type, const uint64_t p_offset,
  ...
  int n = 0;
  n += printf_pick(get_PHDRTYPE(p, p_type), p_type, USE_LT | USE_TAB | SET_PAD(17));
  n += printf_nice(p_offset, USE_FHEX32);
  n += printf_nice(p_vaddr,  USE_FHEX64);
  n += printf_nice(p_paddr,  USE_FHEX64);
  n += printf_nice(p_filesz, USE_FHEX32);
  n += printf_nice(p_memsz,  USE_FHEX32);
...
int dumpelf_programheaders32(const pbuffer_t p, const poptions_t o, Elf32_Ehdr *ehdr) {
  int n = 0;
  n += dump_programheaders0(ehdr->e_phnum);

  for (Elf32_Half i = 0; i < ehdr->e_phnum; ++i) {
    MEMSTACK(Elf32_Phdr, px);
    Elf32_Phdr *p0 = ecget_phdr32byindex(p, px, i);
    if (p0) {
      n += dump_programheaders3(p, p0->p_type, p0->p_offset, p0->p_vaddr, p0->p_paddr, p0->p_filesz, p0->p_memsz, p0->p_flags, p0->p_align);
    }
  }
...
static int dump_dynamic1(const pbuffer_t p, const poptions_t o, const uint64_t d_tag, const uint64_t d_un_d_val, const uint64_t sh_link) {
  ...
  } else if (isused(get_DYNTAGDEC(p, d_tag), d_tag)) {
...
static int dump_relocsver32(const pbuffer_t p, const poptions_t o,
                     Elf32_Shdr *shdr, const uint64_t r_info, pversion_t vnames, const size_t maxvnames) {
  int n = 0;
  if (shdr) {
    MEMSTACK(Elf32_Shdr, dx);
    Elf32_Shdr *d0 = ecget_shdr32byindex(p, dx, shdr->sh_link);
...
static int dump_relocsver64(const pbuffer_t p, const poptions_t o,
                     Elf64_Shdr *shdr, const uint64_t r_info, pversion_t vnames, const size_t maxvnames) {
  int n = 0;
  if (shdr) {
    MEMSTACK(Elf64_Shdr, dx);
    Elf64_Shdr *d0 = ecget_shdr64byindex(p, dx, shdr->sh_link);
...
static int dump_versionsym0(const pbuffer_t p, const uint64_t count,
                     const uint64_t sh_name, const uint64_t sh_addr, const uint64_t sh_offset, const uint64_t sh_link) {
  int n = 0;
...
static int dump_notes3(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
  ...
  const int MAXSIZE = isELF64(p) ? 19 : 12;
  ...
  n += printf_text("Start", USE_LT | USE_TAB2 | SET_PAD(MAXSIZE + 4));
  n += printf_text("End", USE_LT | SET_PAD(MAXSIZE));
  n += printf_text("Page Offset", USE_LT | USE_EOL);

  handle_t ns = fgalloc(notes, count * 3 * 8, descsz0 - (count * 3 * 8) + 3, MEMFIND_NOBLOCKSIZE);
  ...
  for (uint64_t i = 0; i < count; i++) {
    ...
    n += printf_text(fgetstring(ns), USE_LT | USE_TAB3);
    n += printf_eol();
  }
...
static int dump_notes5C(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
                     const uint64_t n_descsz, const handle_t notes) {
  int n = 0;
...
static int dump_notes5Darm(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
  ...
  int n = 0;
//  int32_t uregs[18];
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "R0");
...
static int dump_notes5Dmips(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
                     const uint64_t n_descsz, const handle_t notes) {
  int n = 0;
//  uint32_t pad[6];
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE0");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE1");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE2");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE3");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE4");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE5");
//  uint32_t uregs[32];
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "ZERO");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "AT");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "V0");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "V1");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "A0");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "A1");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "A2");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "A3");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T0");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T1");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T2");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T3");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T4");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T5");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T6");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T7");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S0");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S1");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S2");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S3");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S4");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S5");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S6");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "S7");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T8");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "T9");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "K0");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "K1");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "GP");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SP");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "FP");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "RA");
//  uint32_t hi, lo;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "HI");
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "LO");
//  uint32_t cp0_epc;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "CP0 (EPC)");
//  uint32_t cp0_badvaddr;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "CP0 (BADVADDR)");
//  uint32_t cp0_status;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "CP0 (STATUS)");
//  uint32_t cp0_cause;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "CP0 (CAUSE)");
//  uint32_t unused;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "SPARE6");
...
//  pid_t          pr_sid;
    ...
    if (e_machine == EM_386) {
      ...
      n += dump_notes5Dmips(p, o, e_machine, n_descsz, notes);
...
static int dump_notes6Aarm(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
                     const uint64_t n_descsz, const handle_t notes) {
  int n = 0;
...
//  uint32_t       pr_flag;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "FLAGS");
//  uint16_t       pr_uid;
  n += dump_notes16(p, o, e_machine, n_descsz, notes, "UID");
//  uint16_t       pr_gid;
  n += dump_notes16(p, o, e_machine, n_descsz, notes, "GID");
//  pid_t          pr_pid;
...
static int dump_notes6Ax32(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
                     const uint64_t n_descsz, const handle_t notes) {
  int n = 0;
...
//  signed char    pr_nice;
  n += dump_notes8(p, o, e_machine, n_descsz, notes, "NICE");
//  uint32_t       pr_flag;
  n += dump_notes32(p, o, e_machine, n_descsz, notes, "FLAG");
//  uint16_t       pr_uid;
  n += dump_notes16(p, o, e_machine, n_descsz, notes, "UID");
//  uint16_t       pr_gid;
  n += dump_notes16(p, o, e_machine, n_descsz, notes, "GID");
...
static int dump_notes6Amips(const pbuffer_t p, const poptions_t o, const uint64_t n_descsz, const handle_t notes) {
  int n = 0;
...
static int dump_notes6(const pbuffer_t p, const poptions_t o, const uint64_t e_machine,
  ...
  if (MODE_ISANY(o->action, OPRPROGRAM_COREDUMPDETAILED)) {
    ...
    if (e_machine == EM_386) {
      ...
      n += dump_notes6Amips(p, o, n_descsz, notes);
    } else if (EM_RISCV == e_machine) {
    }
...
//  void    *si_call_addr; /* Address of system call instruction (since Linux 3.5) */
  n += dump_notesNN(p, o, e_machine, n_descsz, notes, "CALL ADDR");
