...
static const int MAXSIZE0 = 24;
static const int MAXSIZE1 = 50;
static const int MAXSIZE2 = 9;
...
int ocdwarf_debug_line(handle_t p, handle_t s, handle_t d) {
  ...
  if (isopcode(p)) {
    ...
    if (MODE_ISANY(oc->ocdump, OPTDWARF_VERBOSE)) {
      n += printf_text("CU header length", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(cu_header_length, USE_FHEX | USE_EOL);
      n += printf_text("Version stamp", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(ws->cu_version_stamp, USE_DEC | USE_EOL);
      n += printf_text("Abbrev offset", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(abbrev_offset, USE_DEC | USE_EOL);
      n += printf_text("Address size", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(address_size, USE_DEC | USE_EOL);
      n += printf_text("Extension size", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(extension_size, USE_DEC | USE_EOL);
      n += printf_text("Type offset", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(type_offset, USE_FHEX | USE_EOL);
      n += printf_text("CU offset size", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(ws->cu_offset_size, USE_DEC | USE_EOL);
      n += printf_text("CU next header offset", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
      n += printf_nice(next_cu_header_offset, USE_FHEX | USE_EOL);
      n += printf_text("CU header type", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
    ...
    if (IS_DLV_OK(x)) {
      if (MODE_ISANY(oc->ocdump, OPTDWARF_VERBOSE)) {
        n += printf_text("Line version", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
        n += printf_nice(line_version, USE_DEC | USE_EOL);
        n += printf_text("Table count", USE_LT | USE_TAB | USE_COLON | SET_PAD(MAXSIZE0));
    ...
    if (MODE_ISANY(oc->ocdump, OPTDWARF_ENHANCED)) {
      ...
      n += printf_text("File name", USE_LT | SET_PAD(MAXSIZE1));
      n += printf_text("Line#", USE_LT);
      n += printf_text("Address", USE_LT | USE_SPACE | SET_PAD(MAXSIZE2));
      n += printf_text("View    Stmt", USE_LT | USE_SPACE);
      n += printf_eol();
    }
