...
static unknown_t ocget_rawshdr(handle_t p, const unknown_t q) {
  if (ismode(p, MODE_OCSHDR)) {
    asection* s0 = ocget(p, MODE_OCSHDR);
    handle_t p0 = ocget(p, OPCODE_RAWDATA);
    if (s0 && p0) {
      if (isELF32(p0))       return ecget_shdr32byoffset(p0, q, s0->filepos);
      else if (isELF64(p0))  return ecget_shdr64byoffset(p0, q, s0->filepos);
    }
  }
...
unknown_t ocget(handle_t p, const imode_t mode) {
  ...
  } else if (ismode(p, MODE_OCSHDR) && OPCODE_RAWDATA == mode) {
    handle_t p0 = ocget(p, OPCODE_PARAM2);
    return isopcode(p0) ? ocget(p0, OPCODE_RAWDATA) : NULL;
  }
  ...
  if (ochas_shdr(p)) {
    if (isopcode(p)) {
      ...
      if (isELF32(p0)) {
        ...
        return ecget_shdr32bytype(p0, sx, type) ? TRUE : FALSE;
      } else if (isELF64(p0)) {
        MEMSTACK(Elf64_Shdr, sx);
        return ecget_shdr64bytype(p0, sx, type) ? TRUE : FALSE;
      }
    }
  }
...
uint64_t ocget_type(handle_t p) {
  if (isopcode(p)) {
    ...
    if (ocisELF32(p)) {
      ...
      MEMSTACK(Elf64_Shdr, sx);
      Elf64_Shdr* s0 = ocget_rawshdr(p, sx);
      return s0 ? s0->sh_type : 0;
    }
...
uint64_t ocget_archsize(handle_t p) {
  if (isopcode(p)) {
    ...
    handle_t p0 = ocget(p, OPCODE_RAWDATA);
    if (isELF32(p0))                     return 32;
    else if (isELF64(p0))                return 64;
...
uint64_t ocget_vaddressbyname(handle_t p, const char* name) {
  if (isopcode(p) && name) {
    ...
    if (isELF32(p0)) {
      MEMSTACK(Elf32_Shdr, sx);
      Elf32_Shdr* s0 = ecget_shdr32byname(p0, sx, name);
      return s0 ? s0->sh_addr : 0;
    } else if (isELF64(p0)) {
      MEMSTACK(Elf64_Shdr, sx);
      Elf64_Shdr* s0 = ecget_shdr64byname(p0, sx, name);
      return s0 ? s0->sh_addr : 0;
    }
  }
...
int ocget_indexbyname(handle_t p, const char* name) {
  if (ismode(p, MODE_OCSHDR)) {
    asection* s0 = ocget(p, MODE_OCSHDR);
    handle_t p0 = ocget(p, OPCODE_RAWDATA);
    return s0 && p0 ? ecget_indexbyname(p0, name) : -1;
  } else if (ismode(p, MODE_OCSHDR32) && name) {
    handle_t p0 = ocget(p, OPCODE_RAWDATA);
...
const char* ocget_namebyoffset(handle_t p, const imode_t mode, const uint64_t offset) {
  if (ismode(p, MODE_OCSHDR)) {
    ...
    if (OPCODE_BYLINK == mode) {
      ...
      if (s0 && p0 && isELF32(p0)) {
        MEMSTACK(Elf32_Shdr, sx);
        Elf32_Shdr* s1 = ecget_shdr32byindex(p0, sx, s0->index + 1);
        return s1 ? ecget_namebyoffset(p0, s1->sh_link, offset) : NULL;
      } else if (s0 && p0 && isELF64(p0)) {
        MEMSTACK(Elf64_Shdr, sx);
        Elf64_Shdr* s1 = ecget_shdr64byindex(p0, sx, s0->index + 1);
        return s1 ? ecget_namebyoffset(p0, s1->sh_link, offset) : NULL;
      }
    } else if (OPCODE_BYDEBUGSTR == mode) {
      return p0 ? ecget_namebyoffset(p0, ecget_indexbyname(p0, ".debug_str"), offset) : NULL;
    }
  } else if (ismode(p, MODE_OCSHDR32)) {
    handle_t p0 = ocget(p, OPCODE_RAWDATA);
    if (OPCODE_BYLINK == mode) {
      Elf32_Shdr* s0 = ocget(p, MODE_OCSHDR32);
      return s0 && p0 ? ecget_namebyoffset(p0, s0->sh_link, offset) : NULL;
    } else if (OPCODE_BYDEBUGSTR == mode) {
      return p0 ? ecget_namebyoffset(p0, ecget_indexbyname(p0, ".debug_str"), offset) : NULL;
    }
  } else if (ismode(p, MODE_OCSHDR64)) {
    handle_t p0 = ocget(p, OPCODE_RAWDATA);
    if (OPCODE_BYLINK == mode) {
      Elf64_Shdr* s0 = ocget(p, MODE_OCSHDR64);
      return s0 && p0 ? ecget_namebyoffset(p0, s0->sh_link, offset) : NULL;
    } else if (OPCODE_BYDEBUGSTR == mode) {
      return p0 ? ecget_namebyoffset(p0, ecget_indexbyname(p0, ".debug_str"), offset) : NULL;
    }
...
bool_t ocget_symbol(handle_t p, uint64_t vaddr, char **name,
  ...
  if (isopcode(p) && name) {
    ...
    if (isodebug(d0)) {
      ...
      if (nline && MODE_ISANY(d0->role, OPDEBUG_LINENO))   *nline = d0->nline;
      if (laddr && MODE_ISANY(d0->role, OPDEBUG_LADDR))    *laddr = d0->laddr;
      if (haddr && MODE_ISANY(d0->role, OPDEBUG_HADDR))    *haddr = d0->haddr;
...
uint16_t ocmake_u16(handle_t p, const uchar_t v0, const uchar_t v1) {
  if (ocisBE(p)) {
    return MODE_PUT1(CAST(uint16_t, v0)) | MODE_PUT0(CAST(uint16_t, v1));
  }
...
uint32_t ocmake_u32(handle_t p, const uchar_t v0, const uchar_t v1, const uchar_t v2, const uchar_t v3) {
  if (ocisBE(p)) {
    return MODE_PUT3(CAST(uint32_t, v0)) | MODE_PUT2(CAST(uint32_t, v1)) | MODE_PUT1(CAST(uint32_t, v2)) | MODE_PUT0(CAST(uint32_t, v3));
  }
...
uint64_t ocmake_u64(handle_t p, const uchar_t v0, const uchar_t v1, const uchar_t v2, const uchar_t v3,
  ...
  if (ocisBE(p)) {
    return MODE_PUT7(CAST(uint64_t, v0)) | MODE_PUT6(CAST(uint64_t, v1)) | MODE_PUT5(CAST(uint64_t, v2)) | MODE_PUT4(CAST(uint64_t, v3)) |
           MODE_PUT3(CAST(uint64_t, v4)) | MODE_PUT2(CAST(uint64_t, v5)) | MODE_PUT1(CAST(uint64_t, v6)) | MODE_PUT0(CAST(uint64_t, v7));
  }
